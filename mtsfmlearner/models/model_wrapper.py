...
from mtsfmlearner.datasets.transforms import get_transforms
from mtsfmlearner.utils.depth import inv2depth, post_process_inv_depth, compute_depth_metrics
from mtsfmlearner.utils.horovod import print0, world_size, rank, on_rank_0
from mtsfmlearner.utils.image import flip_lr
from mtsfmlearner.utils.load import load_class, load_class_args_create, \
    load_network, filter_args
from mtsfmlearner.utils.logging import pcolor
from mtsfmlearner.utils.reduce import all_reduce_metrics, reduce_dict, \
    create_dict, average_loss_and_metrics
from mtsfmlearner.utils.save import save_depth
from mtsfmlearner.models.model_utils import stack_batch
...
def setup_depth_net(config, prepared, **kwargs):
    ...
    depth_net = load_class_args_create(config.name,
        paths=['mtsfmlearner.networks.depth',],
        args={**config, **kwargs},
    )
...
def setup_pose_net(config, prepared, **kwargs):
    ...
    pose_net = load_class_args_create(config.name,
        paths=['mtsfmlearner.networks.pose',],
        args={**config, **kwargs},
    )
    if not prepared and config.checkpoint_path is not '':
...
def setup_model(config, prepared, image_shape=(192, 640), **kwargs):
    ...
    model = load_class(config.name, paths=['mtsfmlearner.models',])(
        **{**config.loss, **kwargs})
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        ...
        if config.dataset[i] == 'KITTI':
            from mtsfmlearner.datasets.kitti_dataset import KITTIDataset
